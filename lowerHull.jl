...
function lowerHull(Lc::Vector{Droite})
    i::Int64 = 0
    ...
    Lr::Vector{Droite} = Vector{Droite}(undef,0)
  	...
  	for i in 1:length(Lc)
        P[count] = Point(Lc[i].a,Lc[i].b,i)
        count += 1
  	end # Boucle d'instanciation des points "légitimes"
    P[count] = Point(-10,0,count) # Point "dummy" associé à la droite d'équation x = - 0.1 soit -10x = 1
    P[count + 1] = Point(1,0,count + 1) # Point "dummy" associé à la droite d'équation x = 1
    P[count + 2] = Point(0,-1,count + 2) # Point "dummy" associé à la droite d'équation -y = 1 soit y = -1
...
function essai()
    Lc::Vector{Droite} = [Droite(0,1//6),Droite(0,1//4),Droite(-1,1//3),Droite(1//2,1//6),Droite(7//8,1//8)]
    return lowerHull(Lc)
end
